...
恒久対策手順。**2026-07-26 に全フェーズを完了済み。**

実施結果は末尾の「実施記録」を参照。以降は同種の作業を行う際の参考として残す。
...
> [!IMPORTANT]
> **レプリケーションの方向は PVE が自動で反転させる。** ゲストが動いている
> ノードから待機ノードへ複製するのが正しいため、これは正常な挙動。
> `ha.tf` は `ignore_changes = [target]` でこれを尊重しており、Terraform は
> 「2 ノード間に複製ジョブが存在すること」だけを管理する。
> (この lifecycle が無いとフェイルオーバーのたびに
> `~ target = "pve" -> "pve3" # forces replacement` が出て HA と衝突する)

> [!WARNING]
> **確認待ちの run を放置しないこと。** フェイルオーバー直後の plan は
> 上記のとおり危険な内容 (コンテナの作り直し) を含む。放置するとワークスペース
> がロックされ、後続の run が全て滞留する。適用せず **discard** して、
> state を貼り直してから新しい run を回すこと。
...
全フェーズ完了。**DNS 断は 22 秒のみ**で、その他のゲストは順次停止を伴ったが
実害は無かった。

| Phase | 結果 |
|-------|------|
| 0 | バックアップ 6 件取得、VM 103 の CD-ROM 取り外し |
| 1 | pve2 を ZFS 化 (`rpool_data` 324G) |
| 2 | pve3 を ZFS 化 (152G)、`ct:105` を復元 |
| 3 | pve を ZFS 化 (352G)、全ゲストを移行して復帰 |
| 4 | HA / レプリケーション / ストレージを Terraform 管理下へ |
| 5 | レプリケーション動作確認済み |

### 手順書の誤りとして判明したもの

1. **`--target-storage` は異種ストレージ間の移行に使えない。**
   LXC も VM も `lvmthin` → `zfspool` は不可で、
   `vzdump` + `destroy` + `restore --storage local-zfs` が必要だった。
   ただし ZFS 化後の ZFS 同士なら通常の `pct migrate` / `qm migrate` が使え、
   VM 103 は `--online --with-local-disks` で無停止移行できた。

2. **`pct` / `qm` はゲストをホストしているノードから実行する必要がある。**

3. **`lvcreate` が旧 ext4 シグネチャで対話待ちになる。** `-y -Wy` が必須。

4. **`VM.Replicate` 権限が必要。** 初回 apply が 403 で全滅した。

5. **Phase 4 の変数はコードの `default` で管理すべきだった。**
   ワークスペース変数にする設計だったため設定漏れが起き、コードが `local-lvm`
   を指したまま実機が `local-zfs` になり、plan が全コンテナの破壊・再作成を
   出す危険な状態を招いた。

### 移行後の構成

| ノード | zpool | ARC 上限 | ホストするゲスト |
|--------|-------|----------|------------------|
| pve  | 352G | 2GB | 100 / 101 / 102 / 103 / 104 |
| pve2 | 324G | 1GB | (レプリカ受け入れのみ) |
| pve3 | 152G | 1GB | 105 |
