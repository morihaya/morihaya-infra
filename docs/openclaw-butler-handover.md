...
2. cron/heartbeat で定時通知。**heartbeat も Bedrock 呼び出しを消費する**ので頻度に注意
   - **朝のブリーフィングとして実装済み**(2026-08-12)。毎日 9:00 JST に `#general` へ
     1 通だけ。カレンダー / サブスク更新 / リマインダの 3 系統をまとめる
     (先に入れたサブスク通知は**これに統合した**。cron は 1 本)
   - 判定は家庭情報リポジトリのスクリプトが行い、cron は `--command` で実行するだけ
     なので **Bedrock を一切消費しない**。カレンダーも MCP ではなく API を直叩きして
     いるのはこのため。定時通知を足すときは、エージェントに考えさせる必要が本当に
     あるか先に検討する
   - **最優先の要件は「何も無い日は投稿しない」。** 何もない日に鳴る通知はいずれ
     全員が無視する。3 系統すべてが空ならスクリプトが黙って終了する
   - 手順とハマりどころは
     [docker/openclaw/README.md](../docker/openclaw/README.md#朝のブリーフィング-cron)
